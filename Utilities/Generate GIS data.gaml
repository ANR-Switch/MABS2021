/**
* Name: Generate data from OSM
* Generate data from OSM. 
...
global {
	...
	string dataset_path <- "../includes/OSM";
	string dataset_path_bound <- "../includes/Shapefiles";
	...
	map
	filtering <- ["building"::[], "shop"::[], "historic"::[], "amenity"::[], "sport"::[], "military"::[], "leisure"::[], "office"::[], "waterway"::[], "leisure"::[], "natural"::[], "highway"::[]];
	...
	init {
		...
		if (use_google_map_data) {
			...
			if (file_exists(googlemap_path)) {
				...
				map
				input_values <- user_input("Do you want to download google maps to fill in the data? (warning: risk of being blocked by google!)", [enter("Download data", false), enter("Delay (in s) between two requests", 5.0)]);
		...
		save Building crs: "EPSG:3857" to: dataset_path_bound + "/buildings.shp" type: shp attributes: ["type"::type, "flats"::flats, "height"::height, "levels"::levels];
		save road crs: "EPSG:4326" to: dataset_path_bound + "/roads.shp" type: shp attributes: ["type"::type];
		save waterway crs: "EPSG:3857" to: dataset_path_bound + "/waterways.shp" type: shp attributes: ["type"::type];
		save leisure crs: "EPSG:3857" to: dataset_path_bound + "/leisures.shp" type: shp attributes: ["type"::type];
		save natural crs: "EPSG:3857" to: dataset_path_bound + "/naturals.shp" type: shp attributes: ["type"::type];
	...
	action load_OSM_data {
		...
		create Building from: geom with:
		[building_att::get("building"), shop_att::get("shop"), historic_att::get("historic"), office_att::get("office"), military_att::get("military"), sport_att::get("sport"), leisure_att::get("lesure"), height::float(get("height")), flats::int(get("building:flats")), levels::int(get("building:levels"))];
	...
	//define the url of the satellite image
		...
		int size_y <- 1500;
		string
		rest_link <- "https://dev.virtualearth.net/REST/v1/Imagery/Map/Aerial/?mapArea=" + bottom_right.y + "," + top_left.x + "," + top_left.y + "," + bottom_right.x + "&mapSize=" + size_x + "," + size_y + "&key=AvZ5t7w-HChgI2LOFoy_UF4cf77ypi2ctGYxCgWOLGFwMGIGrsiDpCDCjliUliln";
		...
		string
		rest_link2 <- "https://dev.virtualearth.net/REST/v1/Imagery/Map/Aerial/?mapArea=" + bottom_right.y + "," + top_left.x + "," + top_left.y + "," + bottom_right.x + "&mmd=1&mapSize=" + size_x + "," + size_y + "&key=AvZ5t7w-HChgI2LOFoy_UF4cf77ypi2ctGYxCgWOLGFwMGIGrsiDpCDCjliUliln";
	...
	reflex vectorization {
		...
		//at the end, save the building
			if (not empty(data_google)) {
				save Building crs: "EPSG:3857" to: dataset_path_bound + "/buildings.shp" type: shp attributes: ["type"::type, "flats"::flats, "height"::height, "levels"::levels];
				save road crs: "EPSG:3857" to: dataset_path_bound + "/roads.shp" type: shp attributes: ["type"::type];
				save waterway crs: "EPSG:3857" to: dataset_path_bound + "/waterways.shp" type: shp attributes: ["type"::type];
				save leisure crs: "EPSG:3857" to: dataset_path_bound + "/leisures.shp" type: shp attributes: ["type"::type];
				save natural crs: "EPSG:3857" to: dataset_path_bound + "/naturals.shp" type: shp attributes: ["type"::type];
			}
